...
class MainActivity : AppCompatActivity() {
        ...
        Thread {
            try {
                ...
                runOnUiThread {
                    ...
                    if (remain >= 0) {
                        val mins = remain / 60; val secs = remain % 60
                        blockText.text = "Đang khai thác block #$nextHeight — $percent% (~${mins}m${String.format("%02d", secs)}s)"
                    } else {
                        val over = -remain; val mins = over / 60; val secs = over % 60
    ...
    private fun startBlockProgress() {
        ...
        handler.post(object : Runnable {
            override fun run() { fetchBtcStats(); handler.postDelayed(this, 10000) }
        })
    }

    private fun addStat(key: String, label: String) {
        val tv = TextView(this).apply { text = label; textSize = 11f; setTextColor(Color.GRAY); setPadding(0,8,0,2) }
        val pb = ProgressBar(this, null, android.R.attr.progressBarStyleHorizontal).apply { max = 100; progress = 0; scaleY = 0.6f }
    ...
    private fun showCreateDialog() {
        ...
        AlertDialog.Builder(this).setTitle("Tạo ví Bitcoin mới").setView(layout).setPositiveButton("Tạo") { _, _ ->
            val name = nameInput.text.toString().trim(); val p1 = passInput.text.toString(); val p2 = pass2Input.text.toString()
    ...
    private fun showImportDialog() {
        ...
        AlertDialog.Builder(this).setTitle("Import ví").setView(layout).setPositiveButton("Import") { _, _ ->
            val name = nameInput.text.toString().trim(); val seed = seedInput.text.toString().trim(); val pass = passInput.text.toString()
    ...
    private fun showMainWallet() {
        ...
        rootLayout.addView(walletNameText); rootLayout.addView(balanceText); rootLayout.addView(priceText); rootLayout.addView(syncText); rootLayout.addView(syncProgressBar); rootLayout.addView(blockText); rootLayout.addView(blockProgressBar); rootLayout.addView(addressText); rootLayout.addView(Space(this).apply { layoutParams = LinearLayout.LayoutParams(1, 20) }); rootLayout.addView(btnRow1); rootLayout.addView(btnRow2); rootLayout.addView(statsTitle); rootLayout.addView(statsContainer); rootLayout.addView(txTitle); rootLayout.addView(txListView)
        addStat("mined", "Đã khai thác"); addStat("halving", "Halving"); addStat("reward", "Phần thưởng"); addStat("diff", "Difficulty"); addStat("mempool", "Mempool"); addStat("hash", "Hashrate"); addStat("fee", "Phí"); addStat("today", "Hôm nay"); addStat("supply", "Cung"); addStat("height", "Height")
        ...
        refreshWallet(); startAutoPriceSync(); startBlockProgress()
    }
    ...
    private fun showSeedDialog() { val pass = EditText(this).apply { inputType = InputType.TYPE_CLASS_TEXT or InputType.TYPE_TEXT_VARIATION_PASSWORD; transformationMethod = PasswordTransformationMethod.getInstance() }; AlertDialog.Builder(this).setTitle("Nhập mật khẩu để xem seed").setView(pass).setPositiveButton("Xem") { _, _ -> val id = walletManager.getActiveId()?: return@setPositive