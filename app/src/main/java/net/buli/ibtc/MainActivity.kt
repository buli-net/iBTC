...
class MainActivity : AppCompatActivity() {
    ...
    override fun onUserInteraction() { super.onUserInteraction(); lastInteractionTime = System.currentTimeMillis() }
    override fun onPause() { super.onPause(); lastInteractionTime = System.currentTimeMillis() }
    override fun onResume() { super.onResume(); lastInteractionTime = System.currentTimeMillis(); if (walletManager.getActive()!= null) refreshWallet() }
    override fun onDestroy() { handler.removeCallbacksAndMessages(null); try { walletManager.lock(); walletManager.stop() } catch (_: Exception) {}; super.onDestroy() }

    private fun setupRootLayout() {
        rootLayout = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(24); layoutParams = ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT) }
        scrollView = ScrollView(this).apply { layoutParams = ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT); addView(rootLayout) }
    }
        ...
        handler.postDelayed(object : Runnable {
            override fun run() {
                ...
                if (active!= null && System.currentTimeMillis() - lastInteractionTime > AUTO_LOCK_MS) {
                    walletManager.lock()
                    runOnUiThread { Toast.makeText(this@MainActivity, "Tự động khóa sau 2 phút không dùng", Toast.LENGTH_SHORT).show(); showUnlockDialog() }
                }
    ...
    private fun showWelcome() {
        ...
        val title = TextView(this).apply { text = "iBTC Wallet v4.1"; textSize = 26f; typeface = Typeface.DEFAULT_BOLD; gravity = Gravity.CENTER; setTextColor(titleColor) }
        ...
        rootLayout.addView(logo); rootLayout.addView(title); rootLayout.addView(subtitle); rootLayout.addView(createBtn); rootLayout.addView(space); rootLayout.addView(importBtn)
    }
    ...
    private fun showCreateDialog() {
        ...
        layout.addView(nameInput); layout.addView(passInput); layout.addView(pass2Input); layout.addView(warning)
        AlertDialog.Builder(this).setTitle("Tạo ví Bitcoin mới").setView(layout).setPositiveButton("Tạo") { _, _ ->
            ...
            try { walletManager.create(name, p1); Thread { walletManager.init() }.start(); toast("Tạo ví thành công"); showMainWallet() } catch (e: Exception) { toast("Lỗi: ${e.message}") }
    ...
    private fun showImportDialog() {
        ...
        layout.addView(nameInput); layout.addView(seedInput); layout.addView(passInput)
        AlertDialog.Builder(this).setTitle("Import ví").setView(layout).setPositiveButton("Import") { _, _ ->
            ...
            if (info == null) { toast("Seed không hợp lệ (cần 12-24 từ)") } else { Thread { walletManager.init() }.start(); toast("Import thành công"); showMainWallet() }
        }.setNegativeButton("Hủy", null).show()
    }
    ...
    private fun showUnlockDialog() {
        ...
        unlockBtn.setOnClickListener { val pass = passInput.text.toString(); if (walletManager.unlock(id, pass)) { Thread { walletManager.init() }.start(); showMainWallet() } else { toast("Sai mật khẩu (khóa sau 5 lần)"); passInput.text.clear() } }
        layout.addView(title); layout.addView(passInput); layout.addView(unlockBtn); rootLayout.addView(layout)
    }
    ...
    private fun showMainWallet() {
        ...
        syncText = TextView(this).apply { text = "Đang kết nối..."; textSize = 13f; setTextColor(subColor) }
        ...
        walletManager.onProgress { pct, txt -> runOnUiThread { syncText.text = txt; syncProgressBar.progress = pct; isSyncing = pct < 100 } }
        refreshWallet()
    }
        ...
        Thread {
            try {
                ...
                runOnUiThread {
                    ...
                    val adapter = object : ArrayAdapter<String>(this, android.R.layout.simple_list_item_2, android.R.id.text1, txs.map { tx -> val type = if (tx.type == "Nhận") "⬇" else "⬆"; "$type ${tx.type} ${String.format("%.8f", tx.amount)} BTC" }) {
            ...
            } catch (e: Exception) { runOnUiThread { toast("Lỗi refresh: ${e.message}") } }
        }.start()
    }
    ...
    private fun showReceiveDialog() {
        ...
        copyBtn.setOnClickListener { val cm = getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager; cm.setPrimaryClip(ClipData.newPlainText("btc_address", address)); toast("Đã copy - sẽ tự xóa sau 30 giây"); handler.postDelayed({ try { cm.clearPrimaryClip() } catch (_: Exception) {} }, 30000) }
    ...
    private fun showSendDialog() { if (isSyncing) { toast("Đang sync, vui lòng đợi"); return }; val layout = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(30) }; val toInput = EditText(this).apply { hint = "Địa chỉ BTC (bc1... hoặc 1... hoặc 3...)" }; val amountInput = EditText(this).apply { hint = "Số lượng BTC"; inputType = InputType.TYPE_CLASS_NUMBER or InputType.TYPE_NUMBER_FLAG_DECIMAL }; val feeRates = try { walletManager.getFeeRates() } catch (_: Exception) { FeeRates(5, 10, 20) }; val feeGroup = RadioGroup(this); val rSlow = RadioButton(this).apply { text = "Chậm ~60 phút (${feeRates.slow} sat/vB)"; id = 1 }; val rNormal = RadioButton(this).apply { text = "Thường ~30 phút (${feeRates.normal} sat/vB)"; id = 2; isChecked = true }; val rFast = RadioButton(this).apply { text = "Nhanh ~10 phút (${feeRates.fast} sat/vB)"; id = 3 }; feeGroup.addView(rSlow); feeGroup.addView(rNormal); feeGroup.addView(rFast); layout.addView(toInput); layout.addView(amountInput); layout.addView(TextView(this).apply { text = "Chọn phí mạng:"; setPadding(0,20,0,0) }); layout.addView(feeGroup); AlertDialog.Builder(this).setTitle("Gửi BTC").setView(layout).setPositiveButton("Tiếp tục") { _, _ -> val to = toInput.text.toString().trim(); val amt = amountInput.text.toString().toDoubleOrNull()?: 0.0; if (to.length < 26 || amt <= 0) { toast("Địa chỉ hoặc số tiền không hợp lệ"); return@setPositiveButton }; val fee = when (feeGroup.checkedRadioButtonId) { 1 -> feeRates.slow; 3 -> feeRates.fast; else -> feeRates.normal }; val estFee = walletManager.estimateFee(to, amt, fee); confirmSend(to, amt, fee, estFee) }.setNegativeButton("Hủy", null).show() }

    private fun confirmSend(to: String, amt: Double, feeRate: Int, estFee: Double) { val layout = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(30) }; val summary = TextView(this).apply { text = "Gửi: $amt BTC\nĐến: $to\nPhí: ~$estFee BTC\nTổng: ${amt + estFee} BTC"; setPadding(0,0,0,20) }; val passInput = EditText(this).apply { hint = "Nhập mật khẩu để xác nhận"; inputType = InputType.TYPE_CLASS_TEXT or InputType.TYPE_TEXT_VARIATION_PASSWORD; transformationMethod = PasswordTransformationMethod.getInstance() }; layout.addView(summary); layout.addView(passInput); AlertDialog.Builder(this).setTitle("Xác nhận").setView(layout).setPositiveButton("Gửi ngay") { _, _ -> val id = walletManager.getActiveId()?: return@setPositiveButton; if (!walletManager.unlock(id, passInput.text.toString())) { toast("Sai mật khẩu"); return@setPositiveButton }; Thread { val result = walletManager.send(to, amt, feeRate); runOnUiThread { if (result.startsWith("Lỗi")) toast(result) else { toast("Đã gửi! TXID: ${result.take(12)}..."); refreshWallet() } } }.start() }.setNegativeButton("Hủy", null).show() }

    private fun showSettings() { val items = arrayOf("👁 Xem seed phrase", "🔑 Đổi mật khẩu", "✏️ Đổi tên ví", "🗑 Xóa ví vĩnh viễn", "🔒 Khóa ví ngay", "ℹ️ Thông tin"); AlertDialog.Builder(this).setTitle("Cài đặt").setItems(items) { _, w -> when(w) { 0 -> showSeedDialog(); 1 -> showChangePassDialog(); 2 -> showRenameDialog(); 3 -> showDeleteDialog(); 4 -> { walletManager.lock(); showUnlockDialog() }; 5 -> showInfo() } }.show() }
    private fun showSeedDialog() { val pass = EditText(this).apply { inputType = InputType.TYPE_CLASS_TEXT or InputType.TYPE_TEXT_VARIATION_PASSWORD; transformationMethod = PasswordTransformationMethod.getInstance() }; AlertDialog.Builder(this).setTitle("Nhập mật khẩu để xem seed").setView(pass).setPositiveButton("Xem") { _, _ -> val id = walletManager.getActiveId()?: return@setPositiveButton; if (walletManager.unlock(id, pass.text.toString())) { val seed = walletManager.getSeed(); val tv = TextView(this).apply { text = seed; textSize = 16f; setTextIsSelectable(true); setPadding(40,40,40,40); gravity = Gravity.CENTER }; AlertDialog.Builder(this).setTitle("⚠️ KHÔNG CHIA SẺ SEED").setView(tv).setPositiveButton("Copy 30s") { _, _ -> val cm = getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager; cm.setPrimaryClip(ClipData.newPlainText("seed", seed)); handler.postDelayed({ cm.clearPrimaryClip() }, 30000) }.setNegativeButton("Đóng", null).show() } else toast("Sai mật khẩu") }.show() }
