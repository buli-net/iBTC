...
class MainActivity : AppCompatActivity() {
    ...
    private fun startBlockProgress() {
        ...
        handler.post(object : Runnable {
            override fun run() {
                fetchBlockUpdate()
                handler.postDelayed(this, 2000) // SỬA: 5000 -> 2000
            }
        })
        handler.post(object : Runnable {
            override fun run() {
    ...
    private fun showUnlockDialog() {
        val id = walletManager.getActiveId(); if (id == null) { showWelcome(); return }
        rootLayout.removeAllViews()
        val isDark = (resources.configuration.uiMode and Configuration.UI_MODE_NIGHT_MASK) == Configuration.UI_MODE_NIGHT_YES
        val titleColor = if (isDark) Color.WHITE else Color.BLACK
    ...
    private fun showMainWallet() {
        ...
        rootLayout.addView(walletNameText); rootLayout.addView(balanceText); rootLayout.addView(priceText); rootLayout.addView(syncText); rootLayout.addView(syncProgressBar); rootLayout.addView(addressText)
        rootLayout.addView(Space(this).apply { layoutParams = LinearLayout.LayoutParams(1, 20) })
        rootLayout.addView(btnRow1); rootLayout.addView(btnRow2); rootLayout.addView(statsTitle); rootLayout.addView(blockText); rootLayout.addView(blockProgressBar); rootLayout.addView(statsContainer); rootLayout.addView(txTitle); rootLayout.addView(txListView)
    ...
    private fun refreshWallet() {
        ...
        Thread {
            try {
                runOnUiThread { syncProgressBar.progress = 30 }; val bal = walletManager.getBalance()
                runOnUiThread { syncText.text = "Đang tải giá BTC..."; syncProgressBar.progress = 60 }; val price = walletManager.price()
                runOnUiThread { syncText.text = "Đang cập nhật địa chỉ..."; syncProgressBar.progress = 85 }; val addr = walletManager.getAddress(); val txs = walletManager.getTransactions()
                runOnUiThread {
                    ...
                    val adapter = object : ArrayAdapter<String>(this, android.R.layout.simple_list_item_2, android.R.id.text1, txs.map { "" }) {
                        override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
                            val view = super.getView(position, convertView, parent); val tx = txs[position]
                            val text1 = view.findViewById<TextView>(android.R.id.text1); val text2 = view.findViewById<TextView>(android.R.id.text2)
                            val isDark = (resources.configuration.uiMode and Configuration.UI_MODE_NIGHT_MASK) == Configuration.UI_MODE_NIGHT_YES
                            ...
                            text2.setTextColor(Color.GRAY); text2.textSize = 11f; return view
                        }
                    }
                    txListView.adapter = adapter; isSyncing = false
                }
            } catch (e: Exception) { runOnUiThread { syncText.text = "Lỗi đồng bộ"; syncProgressBar.progress = 0; isSyncing = false } }
        }.start()
    }

    private fun showReceiveDialog() {
        val address = walletManager.getAddress(); if (address.isEmpty()) { toast("Ví chưa sẵn sàng"); return }
        val layout = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; gravity = Gravity.CENTER; setPadding(40) }
        val imageView = ImageView(this).apply { layoutParams = LinearLayout.LayoutParams(512, 512).apply { bottomMargin = 20 } }
        Thread {
            try {
                val writer = QRCodeWriter(); val bitMatrix = writer.encode(address, BarcodeFormat.QR_CODE, 512, 512)
    ...
    private fun showSendDialog() {
        ...
        AlertDialog.Builder(this).setTitle("Gửi BTC").setView(layout).setPositiveButton("Tiếp tục") { _, _ ->
            val to = toInput.text.toString().trim(); val amt = amountInput.text.toString().toDoubleOrNull()?: 0.0
            ...
            val estFee = walletManager.estimateFee(to, amt, fee); confirmSend(to, amt, fee, estFee)
        }.setNegativeButton("Hủy", null).show()
    }
    ...
    private fun confirmSend(to: String, amt: Double, feeRate: Int, estFee: Double) {
        ...
        AlertDialog.Builder(this).setTitle("Xác nhận").setView(layout).setPositiveButton("Gửi ngay") { _, _ ->
            ...
            Thread { val result = walletManager.send(to, amt, feeRate); runOnUiThread { if (result.startsWith("Lỗi")) toast(result) else { toast("Đã gửi! TXID: ${result.take(12)}..."); refreshWallet() } } }.start()
        }.setNegativeButton("Hủy", null).show()
    }
    ...
    private fun showSettings() {
        ...
        AlertDialog.Builder(this).setTitle("Cài đặt").setItems(items) { _, w ->
            when(w) { 0 -> showSeedDialog(); 1 -> showChangePassDialog(); 2 -> showRenameDialog(); 3 -> showDeleteDialog(); 4 -> { walletManager.lock(); showUnlockDialog() }; 5 -> showInfo() }
        }.show()
    }
    ...
    private fun showSeedDialog() {
        ...
        AlertDialog.Builder(this).setTitle("Nhập mật khẩu để xem seed").setView(pass).setPositiveButton("Xem") { _, _ ->
            ...
            if (walletManager.unlock(id, pass.text.toString())) {
                ...
                AlertDialog.Builder(this).setTitle("⚠️ KHÔNG CHIA SẺ SEED").setView(tv).setPositiveButton("Copy 30s") { _, _ -> val cm = getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager; cm.setPrimaryClip(ClipData.newPlainText("seed", seed)); handler.postDelayed({ cm.clearPrimaryClip() }, 30000) }.setNegativeButton("Đóng", null).show()
    ...
    private fun showChangePassDialog() {
        ...
        AlertDialog.Builder(this).setTitle("Đổi mật khẩu").setView(layout).setPositiveButton("Đổi") { _, _ -> val id = walletManager.getActiveId()?: return@setPositiveButton; if (walletManager.changePassword(id, oldP.text.toString(), newP.text.toString())) toast("Đã đổi thành công") else toast("Sai mật khẩu cũ") }.show()
    }

    private fun showRenameDialog() {
        val input = EditText(this).apply { hint = "Tên ví mới"; setText(walletManager.getActive()?.name?: "") }
        AlertDialog.Builder(this).setTitle("Đổi tên").setView(input).setPositiveButton("Lưu") { _, _ -> val id = walletManager.getActiveId()?: return@setPositiveButton; walletManager.rename(id, input.text.toString()); walletNameText.text = input.text.toString(); toast("Đã đổi tên") }.show()
    }
    ...
    private fun showInfo() { AlertDialog.Builder(this).setTitle("iBTC v4.7").setMessage("Build: 2026-05-25\n• Block update 2s\n• Nút Làm mới đứng im").setPositiveButton("OK", null).show() }
